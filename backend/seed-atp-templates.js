...
async function seedATPTemplates() {
  ...
  await prisma.atp_template_items.deleteMany({});
  await prisma.atp_template_sections.deleteMany({});
  await prisma.atp_document_templates.deleteMany({
    where: { template_code: { startsWith: 'TPL-HW-AVIAT' } }
  });
  await prisma.atp_document_templates.deleteMany({
    where: { template_code: { startsWith: 'TPL-SW-AVIAT' } }
  });
  ...
  const hwTemplate = await prisma.atp_document_templates.create({
    data: {
      ...
      template_name: 'ATP MW Hardware v1.0 (AVIAT)',
      category: 'hardware',
      version: '1.0',
  ...
  const hwSections = [
    ...
    {
      name: 'NE Microwave Acceptance Test',
      items: [
        // Site Cleaning
    ...
    {
      name: 'FE Microwave Acceptance Test',
      items: [
        { desc: 'FE Site to be clean and tidy inside cabinet and area', severity: 'minor', evidence: 'photo', scope: ['MW-NEW', 'MW-UPG'] },
        { desc: 'FE Power Cable connected properly to IDU/POE, ODU', severity: 'major', evidence: 'photo', scope: ['MW-NEW', 'MW-UPG'] },
        { desc: 'FE IDU properly installed on rack/cabinet', severity: 'critical', evidence: 'photo', scope: ['MW-NEW', 'MW-RPL'] },
    ...
    },
    {
      ...
      items: [
        ...
        { desc: 'NIMS Database entry completed', severity: 'major', evidence: 'screenshot', scope: ['MW-NEW', 'MW-RPL'] }
      ]
    },
    {
      ...
      items: [
        ...
        { desc: 'Bill of Quantity (BoQ) verified', severity: 'major', evidence: 'document', scope: ['MW-NEW', 'MW-UPG'] },
  ...
  const swTemplate = await prisma.atp_document_templates.create({
    data: {
      ...
      template_name: 'ATP MW Software v1.0 (AVIAT)',
      category: 'software',
      version: '1.0',
      scope: ['MW-UPG', 'MW-SW'],
      form_schema: {},
  ...
  const swSections = [
    {
      name: 'Equipment Inventory Information',
      items: [
    ...
    {
      name: 'S/W License Inventory Information',
      items: [
        { desc: 'Check installed license list matches BoQ', severity: 'major', evidence: 'screenshot', scope: ['MW-UPG', 'MW-SW'] },
        { desc: 'Verify capacity license active (e.g., 50 MB → 400 MB)', severity: 'major', evidence: 'screenshot', scope: ['MW-UPG'] },
    ...
    {
      name: 'Metering & ODU Verification',
      items: [
  ...
  const hwItemCount = hwSections.reduce((acc, s) => acc + s.items.length, 0);
  const swItemCount = swSections.reduce((acc, s) => acc + s.items.length, 0);

  console.log('✅ ATP Templates seeded successfully!');
  console.log(`📋 Hardware Template: ${hwTemplate.template_code} (${hwItemCount} items)`);
  console.log(`💻 Software Template: ${swTemplate.template_code} (${swItemCount} items)`);
}
